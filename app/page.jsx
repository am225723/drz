...
import { BookingExperience, Button, Card, Section } from '../components/ui';
...
const emphasisCards = [
  { title: 'Psychotherapy', label: 'Reflective care', href: '/services/psychotherapy', icon: MessageCircle, image: 'https://images.unsplash.com/photo-1519682337058-a94d519337bc?auto=format&fit=crop&w=1600&q=85', text: 'A reflective space to understand patterns, emotions, relationships, history, and the meaning beneath symptoms.', accent: 'from-[#edf8f1] to-white border-[#9fcf9a]', highlights: ['Insight-oriented therapy', 'Integrated psychiatric care'] },
  { title: 'Medication Management', label: 'Thoughtful prescribing', href: '/services/medication-management', icon: Pill, image: 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?auto=format&fit=crop&w=1600&q=85', text: 'Thoughtful prescribing, medication review, monitoring, and collaborative decision-making within a whole-person care plan.', accent: 'from-[#f3f8f8] to-white border-[#2f8c85]', highlights: ['Medication review', 'Ongoing monitoring'] }
];
...
function MethodCard({ item }) {
  return <Card><div className="p-5"><CheckCircle2 className="h-5 w-5 text-[#2f8c85]" /><h3 className="body-font-heading mt-3 text-base font-semibold normal-case leading-7 tracking-normal text-slate-950">{item}</h3></div></Card>;
}
    ...
    <section className="relative isolate overflow-hidden bg-[#fbfaf7] py-16 sm:py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-14 grid max-w-5xl items-center gap-8 lg:grid-cols-[.82fr_1.18fr]">
          ...
          <Card className="overflow-hidden shadow-xl"><div className="grid gap-0 md:grid-cols-[.48fr_.52fr]"><div className="bg-[#173f42] p-8 text-white"><p className="text-sm font-semibold uppercase tracking-[0.22em] text-[#d6e7c7]">Meet Douglas Zelisko, MD</p><h2 className="mt-3 text-3xl font-semibold text-white">Board-certified integrative psychiatric care.</h2><p className="mt-4 leading-7 text-slate-100">Dr. Douglas Zelisko is a board-certified psychiatrist offering integrative psychiatric care for adults. His work combines psychiatric evaluation, psychotherapy, medication management, and whole-person treatment planning.</p><Button href="/about" variant="light" className="mt-6">Learn More About Dr. Zelisko</Button></div><div className="space-y-6 p-8"><div><h3 className="mb-4 text-lg font-semibold text-slate-950">Credentials</h3><ul className="space-y-3">{credentialItems.map((item) => <CheckItem key={item}>{item}</CheckItem>)}</ul></div><div><h3 className="mb-4 text-lg font-semibold text-slate-950">Conditions Treated</h3><ul className="space-y-3">{conditionItems.map((item) => <CheckItem key={item}>{item}</CheckItem>)}</ul></div></div></div></Card>
        </div>
      </div>
    </section>

    <Section className="bg-[#fbfaf7]"><div className="mx-auto mb-12 max-w-3xl text-center"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.22em] text-[#2f8c85]">Featured services</p><h2 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Therapy and medication management, thoughtfully integrated</h2><p className="mt-4 text-lg leading-8 text-slate-700">Reflective therapeutic work and careful psychiatric medication decisions can be held together in one clinical relationship.</p></div><div className="grid gap-8 md:grid-cols-2">{emphasisCards.map((card) => { const Icon = card.icon; return <Link key={card.title} href={card.href} aria-label={`Learn more about ${card.title}`} className={`group block overflow-hidden rounded-[2.25rem] border bg-gradient-to-br ${card.accent} shadow-[0_26px_70px_-42px_rgba(23,63,66,.65)] transition duration-300 active:scale-[0.985] md:hover:-translate-y-2 md:hover:shadow-[0_36px_90px_-45px_rgba(23,63,66,.8)]`}><div className="relative h-64 overflow-hidden bg-[#173f42] sm:h-72"><img src={card.image} alt={`${card.title} visual`} loading="eager" decoding="async" referrerPolicy="no-referrer" className="absolute inset-0 h-full w-full object-cover transition duration-700 md:group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-br from-[#102a2f]/58 via-[#173f42]/22 to-transparent" /><div className="absolute left-6 top-6 rounded-full border border-white/30 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[#173f42] backdrop-blur">{card.label}</div><div className="relative z-10 flex h-full items-end justify-between gap-4 p-7"><div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-white text-[#173f42] shadow-xl"><Icon className="h-8 w-8" /></div><div className="rounded-full bg-white/70 px-4 py-2 text-sm font-semibold text-[#173f42] backdrop-blur">Core service</div></div></div><div className="p-8 sm:p-9"><h3 className="text-3xl font-semibold tracking-tight text-slate-950">{card.title}</h3><p className="mt-4 text-lg leading-8 text-slate-700">{card.text}</p><div className="mt-6 grid gap-3 sm:grid-cols-2">{card.highlights.map((item) => <div key={item} className="flex items-center gap-2 rounded-2xl border border-[#dbe7e1] bg-white/80 px-4 py-3 text-sm font-semibold text-[#173f42]"><CheckCircle2 className="h-4 w-4 shrink-0 text-[#2f8c85]" />{item}</div>)}</div><div className="mt-7 flex min-h-[48px] items-center justify-between rounded-2xl border border-[#dbe7e1] bg-white/85 px-5 py-3 text-sm font-semibold text-[#173f42]"><span>Learn more</span><ArrowRight className="h-5 w-5 text-[#2f8c85] transition md:group-hover:translate-x-1" /></div></div></Link>; })}</div></Section>

    <Section className="bg-white"><div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr]"><PhotoPanel image={STOCK_IMAGES.journalDesk} eyebrow="Whole-person care" title="Psychiatric care with time to understand the full picture." text="Care considers symptoms, history, relationships, lifestyle, medical context, and goals for treatment." /><div><p className="mb-3 text-sm font-semibold uppercase tracking-[0.22em] text-[#2f8c85]">The practice</p><h2 className="text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">Thoughtful psychiatric care that goes beyond a brief medication visit.</h2><p className="mt-6 text-lg leading-8 text-slate-700">Care focuses on adults navigating anxiety, depression, grief, relationship issues, trauma, focus concerns, physical health challenges, and complex life transitions. Dr. Z primarily practices psychodynamic psychotherapy while drawing from other modalities when they better fit the person and the moment.</p><div className="mt-8 grid gap-4 sm:grid-cols-2">{METHODS.slice(0, 6).map((item) => <MethodCard key={item} item={item} />)}</div></div></div></Section>
