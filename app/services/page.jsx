...
export default function ServicesPage() {
  return <>
    ...
    <Section className="bg-white"><SectionHeading eyebrow="Featured services" title="Therapy and medication management, thoughtfully integrated" subtitle="Reflective therapeutic work and careful psychiatric medication decisions can be held together in one clinical relationship." /><div className="grid gap-6 md:grid-cols-2">{featuredServices.map((service) => { const Icon = service.icon; return <Link key={service.href} href={service.href} className="group block"><Card className={`h-full overflow-hidden border bg-gradient-to-br ${service.tone} transition hover:-translate-y-1 hover:shadow-xl`}><div className="relative h-60 overflow-hidden bg-slate-950"><img src={service.image} alt={`${service.title} visual`} loading="lazy" decoding="async" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-br from-black/62 via-black/24 to-transparent" /><div className="absolute inset-x-0 bottom-0 h-2/3 bg-gradient-to-t from-black/68 via-black/30 to-transparent" /><div className="relative z-10 flex h-full items-end p-5"><div className="flex items-center gap-4 rounded-[1.5rem] border border-white/15 bg-black/32 p-4 pr-5 text-white shadow-xl backdrop-blur-[2px]"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white text-[#173f42] shadow-lg"><Icon className="h-7 w-7" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/85">Featured service</p><h3 className="mt-1 text-xl font-semibold text-white drop-shadow">{service.title}</h3></div></div></div></div><div className="p-8"><h2 className="text-2xl font-semibold text-slate-950">{service.title}</h2><p className="mt-3 text-lg leading-8 text-slate-700">{service.text}</p><div className="mt-6 inline-flex items-center text-sm font-semibold text-[#173f42]">Read more <ArrowRight className="ml-2 h-4 w-4 transition group-hover:translate-x-1" /></div></div></Card></Link>; })}</div></Section>
