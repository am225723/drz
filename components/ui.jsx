...
export function BookingExperience({ mode = 'all', defaultKey = 'evaluation-virtual' }) {
  const options = mode === 'new' ? BOOKING_OPTIONS.slice(0, 2) : mode === 'current' ? [BOOKING_OPTIONS[3], BOOKING_OPTIONS[2]] : BOOKING_OPTIONS;
  const isPairedPage = mode === 'new' || mode === 'current';
  const gridClass = isPairedPage ? 'mx-auto grid max-w-4xl grid-cols-2 gap-3 sm:gap-5' : 'grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4';
  const [selectedKey, setSelectedKey] = useState(defaultKey);
  const selected = options.find((o) => o.key === selectedKey) || options[0];
  return <div className="relative"><div className={gridClass}>{options.map((option) => { const Icon = option.icon; return <button type="button" key={option.key} onClick={() => setSelectedKey(option.key)} className={`group relative min-w-0 overflow-hidden rounded-[1.5rem] border p-[1px] text-left shadow-xl transition hover:-translate-y-1 sm:rounded-[2rem] ${selected.key === option.key ? 'border-[#9fcf9a]' : 'border-slate-200'}`}><div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${option.tone}`} /><div className="relative h-full rounded-[1.45rem] bg-white p-4 sm:rounded-[1.95rem] sm:p-6"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${option.tone} text-white shadow-lg sm:h-14 sm:w-14`}><Icon className="h-5 w-5 sm:h-7 sm:w-7" /></div><h3 className="mt-4 text-base font-semibold leading-snug text-slate-950 sm:mt-6 sm:text-2xl">{option.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600 sm:mt-3 sm:text-base sm:leading-7">{option.subtitle}</p><div className="mt-4 flex items-center justify-between gap-2 text-xs font-semibold text-[#2f8c85] sm:mt-6 sm:text-sm"><span>{selected.key === option.key ? 'Selected' : 'Select'}</span><ArrowRight className="h-4 w-4 shrink-0" /></div></div></button>; })}</div><div className="mt-8"><IntakeQWidget option={selected} /></div></div>;
}
