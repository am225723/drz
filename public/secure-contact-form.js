    ...
    return `
      <section class="rz-contact-shell" id="secure-contact-form">
        <div class="rz-contact-head">
          ...
          <p>Use this form for non-urgent appointment and administrative requests. Submissions are sent to the Hostinger PHP backend at <code>/api/contact.php</code>.</p>
        ...
        <form class="rz-form" id="rz-secure-contact-form" data-secure-endpoint="/api/contact.php">
          <input type="text" name="website" autocomplete="off" tabindex="-1" aria-hidden="true" style="position:absolute;left:-9999px;opacity:0" />
          ...
          <div class="rz-submit-row">
            <p class="rz-note">The backend stores submissions only if configured on Hostinger. SMS notifications are neutral and should not include clinical details.</p>
          ...
          <div class="rz-admin-note"><strong>Backend:</strong> this form posts to <code>/api/contact.php</code>. Add <code>api/config.php</code> on Hostinger with your Quo/OpenPhone settings before launch.</div>
        </form>
      </section>`;
  }
  ...
  function attachFormBehavior(root) {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        status.textContent = 'Thank you. Your inquiry was received. The practice will follow up using your selected contact preference.';
